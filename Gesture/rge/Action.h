...
public:
	...
	void apply(Frame* f, int time, bool loop, void blendFunc(rgeScalar&, rgeScalar), rgeScalar factor) const {
        ...
        rgeScalar v = getValue(time, loop) * factor;
		if(dst_val)
			blendFunc(*dst_val, v);
	}
...
*/		
	...
	static void addFunc(rgeScalar& a, rgeScalar b)
	{
		a += b;
	}

	static void replaceFunc(rgeScalar& a, rgeScalar b)
	{
		a = b;
	}
	
    static void clearFunc(rgeScalar& a, rgeScalar b)
    {
        a = 0;
    }
	...
	void apply(Frame* f, int time, rgeScalar factor = 1.f, BLEND_FUNC func = REPLACE)
	{
		...
		for(StrToValueDriver::const_iterator i=drivers.begin(); i!=drivers.end(); ++i) {
            ...
            if(func == REPLACE) {
                ...
                d.apply(f, time, false, clearFunc, factor);
            }
		}
	}
