...
#define PADDR(functype, funcname) \
  ...
  if (error) fprintf(stdout, "Could not obtain all of entry points.\n");
#endif
...
int readShaderSource(GLuint shader, const char *file)
{
  ...
  if (source == NULL) {
    fprintf(stdout, "Could not allocate read buffer.\n");
    return -1;
  }
...
void printShaderInfoLog(GLuint shader)
{
  ...
  if (bufSize > 1) {
    ...
    if (infoLog != NULL) {
      ...
      fprintf(stdout, "InfoLog:\n%s\n\n", infoLog);
      free(infoLog);
    }
    else
      fprintf(stdout, "Could not allocate InfoLog buffer.\n");
  }
}
